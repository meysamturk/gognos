...
if(isset($_POST['delete_student']))
{
    ...
    $query = "DELETE FROM `order` WHERE id='$student_id' ";
...
{
    $student_id = mysqli_real_escape_string($conn, $_POST['send_student']);
    $query = "INSERT INTO `tavoni_pdf` (name,number,price_total) select name,number,price_total FROM `order` WHERE id='$student_id' ";
    $query_run = mysqli_query($conn, $query);
    ...
    if($query_run)
    {
        ...
        $query = "DELETE FROM `order` WHERE id='$student_id' ";
        $query_run = mysqli_query($conn, $query);
        $_SESSION['message'] = " با موفقیت انجام شد";
        header("Location: index.php");
        exit(0);
    }
    else
    {
        $_SESSION['message'] = "با موفقیت انجام نشد";
        header("Location: index.php");
        exit(0);
    }
}
